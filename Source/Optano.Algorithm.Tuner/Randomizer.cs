...
namespace Optano.Algorithm.Tuner
{
    ...
    public class Randomizer
    {
        ...
        public IEnumerable<IEnumerable<T>> SplitIntoRandomBalancedSubsets<T>(IReadOnlyList<T> participants, int maximumSubsetSize)
        {
            var numberParticipants = participants.Count;
            var numberSubsets = (int)Math.Ceiling((double)numberParticipants / maximumSubsetSize);
            var shuffledParticipants = this.ChooseRandomSubset(participants, numberParticipants).ToList();

            var balancedSubsetSize = numberParticipants / numberSubsets;
            var numberEnlargedSubsets = numberParticipants % numberSubsets;
            var nextFirstIndex = 0;

            for (var i = 0; i < numberSubsets; i++)
            {
                var currentSubSetSIze = i < numberEnlargedSubsets ? balancedSubsetSize + 1 : balancedSubsetSize;
                yield return shuffledParticipants.Skip(nextFirstIndex).Take(currentSubSetSIze);
                nextFirstIndex += currentSubSetSIze;
            }
        }
