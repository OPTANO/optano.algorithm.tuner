...
namespace Optano.Algorithm.Tuner.GenomeEvaluation.ResultStorage.Messages
{
    ...
    /// <summary>
    /// Message signifying a request for a target algorithm run result.
    /// </summary>
    /// <typeparam name="TInstance">Type of the instances the target algorithm can be run on.</typeparam>
    public class ResultRequest<TInstance>
        where TInstance : InstanceBase
    {
        #region Constructors and Destructors

        /// <summary>
        /// Initializes a new instance of the <see cref="ResultRequest{I}" /> class.
        /// </summary>
        ...
        public ResultRequest(ImmutableGenome genome, TInstance instance)
        {
            // Validate parameters
            if (genome == null)
            {
